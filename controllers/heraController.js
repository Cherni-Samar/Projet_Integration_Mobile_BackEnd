...
exports.requestLeave = async (req, res) => {
  try {
    ...
    if (!employee_id || !employee_email || !type || !start_date || !end_date || !reason ) {
...
// ── Historique actions (avec noms employés) ───────────────────────────────
exports.getRecentActions = async (req, res) => {
  try {
    ...
    // Enrichir avec le nom de l'employé (tous statuts)
    const enriched = await Promise.all(
      actions.map(async (action) => {
        const employee = await Employee.findById(action.employee_id)
          .select('name email role')
          .lean();
        return {
          ...action,
          employee_name: employee?.name || action.details?.name || action.details?.employee_name || 'Employé',
          employee_role: employee?.role || action.details?.role || '',
          employee_email: employee?.email || action.details?.email || ''
        };
      })
    );
...
// ── Supprimer une action (persistant) ─────────────────────────────────────
exports.deleteAction = async (req, res) => {
  try {
    const { action_id } = req.params;
    await HeraAction.findByIdAndDelete(action_id);
...
// ── Tout l'historique (pour "Voir tout") ──────────────────────────────────
  ...
  try {
    ...
    const enriched = await Promise.all(
      actions.map(async (action) => {
        ...
        return {
          ...action,
          employee_name: employee?.name || action.details?.name || 'Employé',
          employee_role: employee?.role || action.details?.role || '',
        };
      })
    );
